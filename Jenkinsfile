...
environment {
    ...
    dockerimage = ''
     ...
     NEXUS_PROTOCOL = "http"
     NEXUS_URL = "http://dev-nexus.trulyafro.com:8081/"
            ...
            steps{
                script{
                    withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId:'nexus-pass', usernameVariable: 'jenkins-user', passwordVariable: 'docker_pass']]) {
                            ...
                            sh "curl -u jenkins-user:$docker_pass http://dev-nexus.trulyafro.com:8081/repository/geolocation/ --upload-file app-${POM_VERSION}.tgz -v"  
                    }
                } 
            }
        }     	    
